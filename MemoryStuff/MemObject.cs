...
public class MemObject
{
    ...
    public void Scan()
    {
        ...
        if (Address.IsValidStaticAddress())
        {
            Console.WriteLine($"{Console.Prefix("Patcher Debug")} Object info for {Console.Value($"'{PropertyName}'")}: {Console.Value($"'{Address:X}'")}:{Console.Value($"'{M.Mem.ReadBytes(Address.ToString("X"), 11).ToHexString()}'")} ");
